...
function loadHugoConfig(projectPath) {
...
var HugoExportPlugin = class extends import_obsidian.Plugin {
  ...
  async applyHugoConfigDefaults() {
    ...
    if (!this.settings.siteBaseUrl && config.baseURL) {
      this.settings.siteBaseUrl = config.baseURL.replace(/\/$/, "");
    ...
    if (!this.settings.defaultAuthor) {
      const author = ((_a = config.params) == null ? void 0 : _a.author) || config.author;
      if (author) {
...
var HugoExportSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    new import_obsidian.Setting(containerEl).setName("Hugo project path").setDesc("Path to your Hugo project root (containing hugo.toml)").addText((text) => text.setPlaceholder("/path/to/hugo/project").setValue(this.plugin.settings.hugoProjectPath).onChange(async (value) => {
      this.plugin.settings.hugoProjectPath = value;
      await this.plugin.saveSettings();
    })).addButton((button) => button.setButtonText("Load defaults").onClick(async () => {
      const result = await this.plugin.applyHugoConfigDefaults();
      if (result.errors.length > 0) {
        new import_obsidian.Notice(result.errors.join("\n"));
      } else if (result.applied.length > 0) {
        new import_obsidian.Notice(`Loaded: ${result.applied.length} settings from Hugo config`);
        this.display();
      } else {
        new import_obsidian.Notice("No empty settings to fill");
      }
    }));
